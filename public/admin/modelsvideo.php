<?php
/**
 * Manage video models (ai_video_models).
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        ModelHelper::logAction('FORM_SUBMIT_VIDEO', ['action' => $action, 'name' => $data['name'], 'cost' => $data['cost_per_image']]);
        if ($action === 'create') {
            $modelManager->createModel($data);
            $message = '✅ مدل ویدئو اضافه شد (هزینه: ' . number_format($data['cost_per_image']) . ' اعتبار).';
        } elseif ($action === 'update' && isset($_POST['id'])) {
            $modelManager->updateModel((int)$_POST['id'], $data);
            $message = '✅ مدل ویدئو بروزرسانی شد (هزینه: ' . number_format($data['cost_per_image']) . ' اعتبار).';
            ...
            <form method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="name" class="form-control" required maxlength="200"
                           ...
                           placeholder="مثلاً: kuaishou/kolors">
                </div>
                <div class="mb-3">
    ...
    <div class="col-md-7">
        <div class="table-container">
            ...
            <table class="table table-hover align-middle">
                <thead><tr><th>ID</th><th>نام نمایشی</th><th>نام مدل</th><th>هزینه</th><th>وضعیت</th><th>عملیات</th></tr></thead>
                <tbody>
                    <?php if (empty($models)): ?>
                        <tr><td colspan="6" class="text-center text-muted">هیچ مدلی یافت نشد.</td></tr>
